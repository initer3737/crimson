//@ts-nocheck
type t_create_custom_el_param={
    Custom_el_name:string
}
type t_dinamycVal1_param={
        class_name:HTMLElement,
        line_error:null|number
    }
//intersection type
type t_class_name_param={
    class_name:string
}
type t_convert_to_pure_css_param=t_class_name_param &{
    prop_val:String,
    result:Function,
    is_raw:boolean
}
type t_register_style_param={
    result:string,
    sisip:null|Function,
    style_id?:string
}
type t_normalize_param={
    regex_patern:RegExp,
    class_name:HTMLElement
}
type t_dicts={
    //keynya apa aja nilainya string
    [key:String]:string
}
type t_data_drawers_param={
    replace?:String|null,
    br?:String|null
}
type t_ImportantData_breakpoint_prop={
    [key:string]:string,
    data_drawers:(param:t_data_drawers_param)=>string,
    // draws_breakpoints:Functio                                                                                                                                                    n
}
type t_prop_val_result_param={
    result:String
}
type t_BpDinval1TranslatorAlgoritm_param={
    g2:string,
    el:null|string|HTMLElement|undefined|object,
    line_error:string|number|null,
    prop_val_result:string|(({result}:t_prop_val_result_param)=>void)
}
type t_variantsPredefinedClassByDevTranslatorAlgoritm_param=t_BpDinval1TranslatorAlgoritm_param
type t_Dinval1TranslatorAlgoritm_param=t_BpDinval1TranslatorAlgoritm_param
type t_Dinval2TranslatorAlgoritm_param=t_BpDinval1TranslatorAlgoritm_param
type t_PureCssTranslatorAlgoritm_param=t_BpDinval1TranslatorAlgoritm_param
type t_Val37PredefinedClassByDevTranslatorAlgoritm_param=t_BpDinval1TranslatorAlgoritm_param
type t_dictionary_filter_by_param={
    key:string
}
/**
     * @desc mengolah algoritme breakpoint   */   
class BreakpointTranslatorAlgoritm{
  ...
  constructor(){
    ...
    this.#breakoints=new Breakpoints()
    ...
    //  */
     BpDinval1TranslatorAlgoritm=({g2,el=null,line_error,prop_val_result})=>{
     
  }
  
}
/**
     * @desc untuk mengolah breakpoints    */   
...
class Breakpoints{
  ...
  breakpoint_regex
  constructor(){
    this.breakpoint_regex=/@[\w]{2,2}\:?\S+/g as RegExp
    this.#strict_checker_breakpoint=/@[\w]{2,2}\:\S+/g
    this.#catchs=this.#catch_data.generateCatch()
  }

  /**
   * @desc breakpoint error checker to notify the webdev
   */
  #breakpoint_error_checker({class_name}:t_class_name_param){
      if(!this.#strict_checker_breakpoint.test(class_name))throw new Error("your beackpoint class must have : like @md:color-red-40")
  }
  /**
   * @desc kalau dia breakpoint[@md:color-blue-90] maka return, prevent breakpoint class to enter 
   */
  prevent_breakpoint_only({class_name}:t_class_name_param){
   ...
   */
  includes_breakpoint_only({class_name}:t_class_name_param){
      this.#breakpoint_error_checker({class_name})
      return !this.breakpoint_regex.test(class_name)
  }
...
/**
    ...
    */   
    class CustomElementCreator{
        ...
        #create_custom_el({Custom_el_name}:t_create_custom_el_param){
...
class MassTranslators{
      ...
      #important_data      
    //   #dicts 
      #dicts: Record<string, { first?: string; [key: string]: any }> = {};
      #style_tags
    constructor(){
     ...
     * @param {(prop_val_result:{result:string})=>string} obj.prop_val_result 
    //  */
     Dinval1TranslatorAlgoritm=({g2,el=null,line_error,prop_val_result}:t_Dinval1TranslatorAlgoritm_param)=>{
        let prop_val=""
              ...
              datas?.map(val=>{
                if(!regex_4_false.test(val))return
                 ...
                 //console.log("valvalval ",val)
                    const get_el=()=>{
                        if(el && typeof el == 'object' && 'getAttribute' in el){
                            const value=typeof el?.getAttribute === "function"? el.getAttribute(this.#important_data.canvas_attr) || el: el
                        }
                    }
                    const attrValue =get_el() 
     ...
     * @param {(prop_val_result:{result:string})=>string} obj.prop_val_result 
    //  */
     Dinval2TranslatorAlgoritm=({g2,el=null,line_error,prop_val_result}:t_Dinval2TranslatorAlgoritm_param)=>{
        let prop_val="";
        type t_Error_display_param={
            el:HTMLElement,
            line_error:number,
            resultat:string,
            message:null|string
        }
          const Error_display=({el,line_error,resultat,message=null}:t_Error_display_param)=>{
     ...
     * @param {(prop_val_result:{result:string})=>string} obj.prop_val_result 
    //  */
     PureCssTranslatorAlgoritm=({g2,el=null,line_error,prop_val_result}:t_PureCssTranslatorAlgoritm_param)=>{
        let prop_val="";
     ...
     * @param {(prop_val_result:{result:string})=>string} obj.prop_val_result 
    //  */
     Val37PredefinedClassByDevTranslatorAlgoritm=({g2,prop_val_result,el=null,line_error}:t_Val37PredefinedClassByDevTranslatorAlgoritm_param)=>{
        let prop_val="";
       const style_tag=Utils.$("#"+this.#important_data.attr_html_tag).textContent
        const dictionary_filter_by=({key}:t_dictionary_filter_by_param)=>{
       ...
       const dicts=dictionary_filter_by({key:"defined"})
          const formated_dicts=dicts.reduce((obj,value,idx)=>{
                obj[value[0]]={...value[1] as t_dicts}
               return obj
     ...
     * @param {(prop_val_result:{result:string})=>string} obj.prop_val_result 
    //  */
     variantsPredefinedClassByDevTranslatorAlgoritm=({el=null,line_error=null,g2,prop_val_result}:t_variantsPredefinedClassByDevTranslatorAlgoritm_param)=>{
        let prop_val="";
...
class Utils {
     ...
     */
      static escape_css_whitespace=({class_name}:t_class_name_param)=>{
        const regex=/\s+/g
     ...
     */
     static escape_css=({class_name}:t_class_name_param)=>{
        const regex=/([\*\&\(\)])/g
   ...
   */
  static convert_to_pure_css({ class_name, prop_val , is_raw=true,result }:t_convert_to_pure_css_param) {
...
`
   ...
   */
  static register_style = ({result,style_id=new ImportantData().attr_html_tag,sisip=null }:t_register_style_param) => {
...
*/
static normalize=({class_name,regex_patern}:t_normalize_param)=>{
    const t =class_name
    if(t != undefined){
       const classData=t.className.replace(regex_patern,(match:string,offset:String,fullStr:string)=>{
...
class ImportantData{
        #datas;
        attr_html_tag:string
        std_prefix:string
        style_tag:string
        canvas_attr:string
        root:string
        custom_elements_style:string
        custom_elements_dinamyc_class:string
        custom_elements_js:string
        custom_elements_var:string
        custom_elements:Array<string>
        breakpoints:t_ImportantData_breakpoint_prop
    constructor(){
        ...
        //todo untuk mempermudah dalam pengaplikasian regex replace nantinya
          data_drawers:function({replace=null,br=null}={}){
            const data={
             xs:`@media only screen and (${this.xs}) {${replace}}`,
             sm:`@media only screen and (${this.sm}) {${replace}}`,
             md:`@media only screen and (${this.md}) {${replace}}`, 
             lg:`@media only screen and (${this.lg}) {${replace}}`,    
             xl:`@media only screen and (${this.xl}) {${replace}}`
            }
              const breakpoint_required=Object.keys(data).map((key)=>key)
               // console.log(breakpoint_required)
                if(br !== null && data[br] === undefined)throw new Error(`break point only ${breakpoint_required} ${br} is unkown `)
              return br!==null?data[br]:data
        },
      //},
          draws_breakpoints:function(){
            //console.log("br is ",this.xs)
            //`/* https://www.w3schools.com/howto/howto_css_media_query_breakpoints.asp*`
              this.data_drawers()
                
      },
          /*
           * @desc untuk mereplace berdasarkan br:breakpoint misal xl lg sm xs 
           * @param {(string|null)} br
           * @param {(string|null)} replace
           * */
      replace_placeholder:function({br,replace}){
          return this.data_drawers({br,replace})
      }
    }
  //console.log(this.breakpoints.replace_placeholder({br:"xl",replace:"[\s\S]+?"}))
      ...
      * {
      ...
      #root, #__next {
        ...
        ${this.breakpoints.draws_breakpoints()}
        `
        this.#datas={
...
// ?our fiture start
    /**
         ...
         * @return void
         */
        // type t_dinamycClass_param={
        //     class_name:string,
        //     line_error:null|number
        // }
        // :t_dinamycClass_param
     dinamycClass({class_name,line_error=null}) {
       ...
       //console.log("checking dincalss ",class_name)
        let classes = class_name?.getAttribute(this.#important_data.canvas_attr).replace(/\w+-\*\s*[^*]+\s*\*/g,(match:string,offset:string,fullstr:string)=>{
      ...
      // console.log("classes ",classes)
       ...
       const lastDinclassdatas:any[] = []
       const lastDinclassresult:[]|undefined = classes?.match(regexs)
       lastDinclassresult?.forEach((val:string)  => {
        ...
        for (const result of lastDinclassdata.matchAll(regexs)) {
            ...
            class_name_result= result[1]
              ...
              const  groupnum2 = result[2]
                ...
                //  ? dinamic val1
             this.#mass_translator.Dinval1TranslatorAlgoritm({el:class_name,line_error,g2:groupnum2,prop_val_result:({result}:{result:string})=>{
            ...
            this.#mass_translator.Dinval2TranslatorAlgoritm({el:class_name,line_error,g2:groupnum2,prop_val_result:({result}:{result:string})=>{
                    prop_val+=result
                    // console.log("dinval2 nih ",result)
             }})
                //? pure css
                this.#mass_translator.PureCssTranslatorAlgoritm({el:class_name,line_error,g2:groupnum2,prop_val_result:({result}:{result:string})=>{
                    prop_val+=result
                   // console.log("pure css ",prop_val)
             }})
          //console.log("ctho eto g2 ",groupnum2)
                //? 37 defined class by dev
            this.#mass_translator.Val37PredefinedClassByDevTranslatorAlgoritm({el:class_name,line_error,g2:groupnum2,prop_val_result:({result}:{result:string})=>{
                    prop_val+=result
                     //console.log("37 by dev ",result)
             }})
            
                    ///10 variants
            this.#mass_translator.variantsPredefinedClassByDevTranslatorAlgoritm({el:class_name,line_error,g2:groupnum2,prop_val_result:({result}:{result:string})=>{
                    prop_val+=result
                    // console.log("37 by dev ",result)
             }})
    ...
    // console.log("propval is ",{prop_val,class_name_result})
        ...
        // aproved by admin
        Utils.convert_to_pure_css({class_name:class_name_result,is_raw:false,prop_val:prop_val,result:({result}:{result:string})=>{
...
*@param {string} obj.class_name 
*/
dinamycVal1({class_name,line_error=null}:t_dinamycVal1_param) { 
    ...
    //  console.log("apa itu ",classNames)
        Utils.normalize({regex_patern:/\w+-\*\s*[^*]+\s*\*/g,class_name:class_name})
             ...
             class_names?.map(class_name=>{
               ...
               this.#mass_translator.Dinval1TranslatorAlgoritm({el:class_name,line_error,g2:class_name,prop_val_result:({result}:{result:String})=>{
                 prop_val+=result
                 //console.log("dinval 1 ",result)
                 // if(!escaped_clsnm || !result)return
                 //console.log(escaped_clsnm,"resultnya ",result)
                 //return  
                 Utils.convert_to_pure_css({class_name:escaped_clsnm,prop_val:prop_val,is_raw:false,result:({result}:{result:String})=>{
...
type t_scanDoms_param={
    val:HTMLElement,
    idx:number
}
        this.#scanDoms(({val,idx}:t_scanDoms_param)=>{
            const index = idx +1
