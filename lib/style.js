...
// ?our fiture start
    /**
     ...
     dinamycClass() {
        ...
        // aproved by admin
        Utils.convert_to_pure_css({class_name:class_name,is_raw:false,prop_val:prop_val,result:({result})=>{
            Utils.register_style({result:result,style_id:"crimsonStyle"})
        }})
...
dinamycVal({class_names}) { 
    ...
    const catchs=this.#catchs
        ...
        Utils.normalize({class_name:class_names})
    // console.log(catchs)        
...
// ?install start
    ...
    install() {
        ...
        this.#dinamycClass()
        this.#scanDoms(({val})=>{
            // Utils.normalize({regex_patern:/\w+-\*[^*]+\*/g,class_name:val})
            new Fiture().dinamycVal({class_names:val})
