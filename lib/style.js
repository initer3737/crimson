...
console.log("what is me ",class_name)
    ...
    if(!cls_name.match(regex_catch_variable_elem)){
      console.warn("variable style not detected, please make sure to write the variable style in the correct format like --nama-variable:value; ")
      return
    }
if(cls_name == undefined)return
    ...
    install() {
        ...
        this.#scanDoms(({val})=>{
            const str_el=val.outerHTML //convert dom into string
              this.#important_data.custom_elements.map(custom_el=>{
                if(str_el.includes(custom_el)){
                  this.#dinamycClass({class_name:val})
                  this.#var_style({class_name:val})
                  console.log("scan doms is ",val)
                }
              })
              try {
