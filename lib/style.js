...
`
  ...
  static register_style = ({result,style_id=new ImportantData().attr_html_tag,sisip=null }) => {
    let style_tag=Utils.$("#"+style_id).textContent   
    ...
    if(typeof sisip === "Function" || sisip === null){
      sisip===null?style_tag+=result:sisip({value:style_tag})
    }else{
      console.warn("sisip argument must be either function or null")
    }
...
class ImportantData{
    ...
    constructor(){
        ...
        this.custom_elements=[this.custom_elements_js,this.custom_elements_style,this.custom_elements_var]
    ...
    /**
     ...
     dinamycClass({class_name}) {
       ...
       if(lastDinclassdatas.length >1){
         ...
         throw new Error(`multiple dynamic class detected, you must use dynamic class only one time in a single element to avoid unexpected behavior, please only use 1 dinamyc class per ${this.#important_data.custom_elements_style} element ${class_name.getAttribute(this.#important_data.canvas_attr)}`)
...
dinamycVal1({class_name}) { 
    ...
    //  console.log("apa itu ",classNames)
        Utils.normalize({regex_patern:/\w+-\*[^*]+\*/g,class_name:class_name})
...
//console.log("why yes ",class_name)
    ...
    const regex_catch_variable_elem=/\w+-\*[^*]+?\*/g
    // ? to search value in the style tag
    ...
    if(is_no_in_caths?.length >0){
        ...
        is_no_in_caths.forEach(val=>{
            ...
            const prop_val=rawdata.replace(/(\w+)-\*([^*]+)\*/g,(match,g1,g2,_,full)=>{
                    ...
                    if(!test1)throw new Error(g1 +" is not valid sintaks")
...
console.log("what is me ",class_name)
    ...
    if(!cls_name.match(regex_catch_variable_elem)){
      console.warn("variable style not detected, please make sure to write the variable style in the correct format like --nama-variable:value; ",class_name)
      return
    }
if(cls_name == undefined)return
            ...
            const prop_val=rawdata.replace(/(\w+)-\*([^*]+)\*/g,(match,g1,g2,_,full)=>{
                    ...
                    const prop_vals=`${test1}:${g2};`
                            if(test1 === undefined){
                              ...
                              throw Error(test1," is not valid sintaks")
                            }
                    return prop_vals
                })
        ...
        this.#scanDoms(({val})=>{
            const str_el=val.outerHTML //convert dom into string
                    try {
                      ...
                      if(str_el.includes(this.#important_data.custom_elements_var)){
                        this.#var_style({class_name:val})
                      }
                      ...
                      console.warn(error.message,val??null);
              ...
              try {
                ...
                this.#dinamycVal1({class_name:val})
              } catch (error) {
                   console.warn(error.message,val);
                            
              }        // console.log("ini scans dom ",val)
        },{})
