       ...
       //console.log("data nih ",object.fromEntries(dicts))
         // check for existed class in style tag
            ...
            existed_in_style_tag?.map(val => {
                ...
                if ( res_val_data == undefined)return //mencegah style seperti blue-90 untuk masuk
                      //if(res_val > 37)console.log(splited_val.join("-"))//validasi untuk 37 defined class lalu throw error
                      if(!style_tag.includes(splited_val.join("-")))throw new Error(`error make sure your style is valid defined class [1 until 37] ${splited_val.join("-")} ${line_error} element after body tag `, el)//validasi untuk 37 defined class lalu throw error
                 ...
                 // }
                  // kalau gak ada 10 variant yang valid maka return 
                    const check=el?.className
                          ...
                          if(check.match(/\w+-\w+-[1-90]/g))return 
                          if(!check.match(/\w+-\w+-[1-90]/g))return 
