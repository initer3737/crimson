...
class MassTranslators{
    ...
    //  */
     Dinval1TranslatorAlgoritm=({g2,el=null,line_error,prop_val_result})=>{
        let prop_val=""
              const regex_4_false=/\w+-\*[^*]+\s*[\S]{1}/g
              const datas=g2.match(regex_4_false)

                //console.log("valnya ",datas)
              datas?.map(val=>{
                   // if(g2
                // console.log("valvalval ",val)
                    const attrValue = typeof el?.getAttribute === "function"? el.getAttribute(this.#important_data.canvas_attr) || el.className: el?.className
       ...
       const validate_regex_dinval1=/([\w]+)-\*\s*([^*]+)\s*\*/g
        let dinamicVal = [...g2.matchAll(validate_regex_dinval1)]
