...
class MassTranslators{
    ...
    //  */
     Dinval2TranslatorAlgoritm=({g2,prop_val_result})=>{
        let prop_val="";
        const dinamicval2 = g2.matchAll(/([\w]+?)-\&([\w]+)\&-\*([^*]+)\*/g)
    ...
    //  */
     PureCssTranslatorAlgoritm=({g2,prop_val_result})=>{
        let prop_val="";
        const purePropValCss = g2.match(/[\w|\w-\w]+\:[^:]+[;$]/g)
        purePropValCss.map(val => { // ?isi propval ke dalam variable propval
            prop_val += val
            // console.log("apa itu ",propVal)
        }),
        // return {prop_val}
        ...
        let prop_val="";
         // check for existed class in style tag
            ...
            existed_in_style_tag?.map(val => {
                ...
                if ( res_val_data == undefined && res_val > 37 )return
...
// ?for convert end
 /**
         ...
         */
  static register_style = ({result }) => {
            
    // console.log(result)
    Utils.$("#"+this.#std_attr_html_tag).textContent+=result
 ...
 */
 class DomScanner {
    ...
    */
        scanDoms(fn) {
            const doms = Utils.All("*[class]:not(script,link,style)")
                doms.forEach(val=>{
                    //  console.log(val)
                    fn({val})
...
// ?create style tag element and generate default style start
    ...
    // ? generate , 0-37 , color 10 -90 5 variant include text and bg
    /**
     ...
     #generate_styles() {
        ...
        for (let i = 0; i <= 37; i++) {
            for(let j=0;j<promised.length;j++){     
                this.#convert_to_pure_css({class_name:`${promised[j][0]}-${i}`,is_raw:false,prop_val:`${promised[j][1].first}:${i}px`,result:(({result})=>{
...
// ?our fiture start
    /**
         ...
         */
     dinamycClass() {
        const classes = "card-*padding:12px 1.12rem; text-transform:capitalize; text-align:center; mt-4 mx-3 pt-5 mb-*0.10rem * mt-*12rem* bg-&clip&-*fixed* bg-&color&-*#ddd* * cardslash-*padding:112px 1.12rem; mv-*13rem* text-transform:capitalize; text-align:center; mt-4 mx-3 pt-5 mb-*0.10rem * mt-*12rem* bg-&clip&-*fixed* bg-&color&-*#ddd* *  mt-*0.25rem* cardslash-* mt-*1.24rem* * cardslash-^bg-blue-90 mt-*4rem* trans:uppercase; bg-&clip&-*fixed*^ cardslash-^ mt-*4rem * trans:uppercase; bg-&clip&-*fixed* mt-*23rem* box-&shadow&-*12px 12px 12px 12px #ddd * bg-blue-90 bg-blue-80   background:red; bg-blue-50 mt-37 mb-16 mb-*56rem* m-&top&-*45px* py-37^"
        const dicts = {
            m:"margin",
            mt: 'margin-top',
            mb: 'margin-bottom',
        ...
        spreaded_variants_datas.map(val=>{
            ...
            //    console.log("lupa apaan ini ",result_variants_regexs[1])
        ...
        // aproved by admin
        this.#convert_to_pure_css({class_name:class_name,is_raw:false,prop_val:prop_val,result:({result})=>{
            this.#register_style({result:result})
        }})
...
dinamycVal({class_names}) { 
    ...
    //  console.log("apa itu ",classNames)
        this.#normalize({regex_patern:/\w+-\*[^*]+\*/g,class_name:class_names})
    //  console.log("valid ",t.className)

    const catchs=this.#catchs
    
        this.#normalize({class_name:class_names})
    // console.log(catchs)        

    const dicts=this.#dict
    ...
    if(is_no_in_caths?.length >0){
        ...
        is_no_in_caths.forEach(val=>{
            ...
            this.#convert_to_pure_css({class_name:escaped_clsnm,prop_val:prop_val,is_raw:false,result:({result})=>{
...
class CrimsonStyle {
    // ? constructor and properties start
        ...
        #style_tag_data
        #std_attr_html_tag
        #dom_scanner=new this.#dom_scanner()
        #important_data=new ImportantData()
        #catch_data=new CatchData()
        #mass_translator=new MassTranslators()
        #catchs;
        #std_prefix =this.#important_data.std_prefix
        #style_tag = this.#important_data.style_tag
    // #style_tag_data=Utils.$("#crimsonStyle")
        #reset_style = this.#important_data.reset_style
...
// ?install start
    ...
    install() {
        ...
        this.#dinamycClass()
        this.#scanDoms()
        // console.log("crimson is "+obj.#std_prefix)
    }
// ?install end
